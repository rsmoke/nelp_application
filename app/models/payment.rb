class Payment < ApplicationRecord
  ...
  scope :for_current_registration_period, ->(date=DateTime.parse('202010010000')) { where(transaction_date: date.strftime("%Y%m%d%H%M")..(date + 11.months).strftime("%Y%m%d%H%M")) }
